...
class Panda():
    def __init__(self):
        ...
        # K: kinetics 設定一些阻力的東西,在set_stiffness（）中用到
        self.K_pos=1000
        self.K_ori=30
        self.K_ns=10
        self.curr_pos=None
        self.curr_ori=None 
        self.attractor_distance_threshold=0.05 # if the attractor distance is larger than the threshold, the robot will wait
    ...
    def go_to_pose(self, goal_pose, interp_dist=0.001, interp_dist_polar=0.001): ##### Are both interpolation distances needed?
        ...
        i=0
        while i  < step_num:
            print("Moving to step:", i+1, "/", step_num)
    ...
    def interactive_gripper(self):
        rospy.loginfo("Interactive gripper control started!")
        rospy.loginfo("Commands:")
        rospy.loginfo("  h - home gripper")
        rospy.loginfo("  o - open gripper (8cm)")
        rospy.loginfo("  c - close gripper (2cm)")
        rospy.loginfo("  g - grasp object (2cm, 10N)")
        rospy.loginfo("  s - stop gripper")
        rospy.loginfo("  q - quit")

        self.set_stiffness(1000, 1000, 1000, 30, 30, 30, 10)  
        try:
            while not rospy.is_shutdown():
                ...
                if cmd == 'h':
                    ...
                    self.grasp_gripper(0.02, 10.0)
                elif cmd == 's':
                    rospy.loginfo("Stopping gripper...")
                    self.stop_gripper()
                elif cmd == 'q':
                    rospy.loginfo("Quitting...")
                    break
                else:
                    rospy.loginfo("Unknown command. Use h/o/c/g/s/q")
                    
                rospy.sleep(0.1)
            
        except KeyboardInterrupt:
            rospy.loginfo("Interactive test interrupted")
        except EOFError:
            rospy.loginfo("Interactive test ended")