...
class RosFemtoCameraWrapper():
    def __init__(self, camera_name="camera", mode="fixed", calibration_result="."):
        rospy.Subscriber("/{}/color/image_raw".format(camera_name), msg_Image, self.rgb_callback1)
        rospy.Subscriber("/{}/depth/image_raw".format(camera_name), msg_Image, self.depth_callback1)
        rospy.Subscriber("/{}/depth/camera_info".format(camera_name), CameraInfo, self.cameraInfo_callback)    
    ...
    def rgb_callback1(self, data, path=None):
        bridge = CvBridge()
        self.rgb_image1 = cv2.cvtColor(np.array(bridge.imgmsg_to_cv2(data, data.encoding)), cv2.COLOR_RGB2BGR)

        # Write to specific folder
        if path is not None:  
            cv2.imwrite( os.path.join( path, "rgb_{}.png".format( int( time.time() * 1000 ) ) ), self.rgb_image1 )
            rospy.loginfo("Image saved to {}".format( os.path.join( path, "rgb_{}.png".format( int( time.time() * 1000 ) ) ) ) )
        if self.record_flag:
